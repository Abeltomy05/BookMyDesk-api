...
async sendRejectionEmail(to: string, reason: string, retryUrl: string | null, entityLabel: string): Promise<void> {
  const subject = `${entityLabel} Rejected - BookMyDesk`;
  ...
  const retrySection = retryUrl
   ...
   const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
      ...
      <h3 style="color: #333;">${entityLabel}</h3>
      <p style="color: #555; font-size: 16px;">We regret to inform you that your ${entityLabel.toLowerCase()} has been rejected.</p>
  ...
  const mailOptions = {
    ...
    to,
    subject,
    text: `Your ${entityLabel.toLowerCase()} has been rejected. Reason: ${reason}`,
    html: htmlContent
  };
